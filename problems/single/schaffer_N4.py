...
class SchafferN4(SingleObjectiveProblem):
    def __init__(self, **kwargs):
        ...
        self.domain = (xl, xu)
        self._pareto_set = np.array([[0, 1.25313],
                                     [0, -1.25313]])
        self._pareto_front = 0.292579
        self._optimum = min
        self._argopt = np.argmin

    ## Overide Methods ##
    def _f(self, X):
        f = 0.5 + (cos(sin(abs(X[0]**2 - X[1]**2)))**2 - 0.5) / (1 + 0.001 * (X[0]**2 + X[1]**2))**2
        return f
